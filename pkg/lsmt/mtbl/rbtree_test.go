...
func NewEntry(k, v string) *binary.Entry {
	if v == "" {
		return &binary.Entry{
			Key: []byte(k),
		}
...
func TestFindNearest(t *testing.T) {
	...
	// print tree
	tree.Scan(func(e *binary.Entry) bool {
		fmt.Printf("%s\n", e)
		return true
	})
...
func makeKey(i int) *binary.Entry {
	return &binary.Entry{
...
func TestRbTree_ScanRange(t *testing.T) {
	...
	tree.ScanRange(start, stop, func(e *binary.Entry) bool {
		if e.Key == nil && bytes.Compare(e.Key, start.Key) == -1 && bytes.Compare(e.Key, stop.Key) == 1 {
