...
// SWAL is a write-ahead log structure
type SWAL struct {
...
// OpenSWAL opens and returns a new write-ahead log structure
func OpenSWAL(c *SWALConfig) (*SWAL, error) {
	// check config
...
func (l *SWAL) CloseAndRemove() error {
	// lock
	l.lock.Lock()
	defer l.lock.Unlock()
...
func (l *SWAL) loadSegmentFile(path string) (*segment, error) {
	...
	s.remaining = defaultMaxSegmentSize - offset
	return s, nil
}
...
func (l *SWAL) makeSegmentFile(index int64) (*segment, error) {
	// create a new file
...
func (l *SWAL) TruncateFront(index int64) error {
	// lock
	l.lock.Lock()
	defer l.lock.Unlock()
...
func (l *SWAL) GetConfig() *SWALConfig {
	// lock
	l.lock.Lock()
	defer l.lock.Unlock()
	return l.conf
