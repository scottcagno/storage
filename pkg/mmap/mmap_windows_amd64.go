...
type Mapping struct {
	...
	// hMappedFile specifies the descriptor of the mapping object provided by the operation system.
	hMappedFile syscall.Handle
...
func Open(fd uintptr, offset int64, length uintptr, mode Mode, flags Flag) (*Mapping, error) {
	...
	m.hMappedFile, err = syscall.CreateFileMapping(m.hFile, nil, prot, maxSizeHigh, maxSizeLow, nil)
	if err != nil {
		return nil, os.NewSyscallError("CreateFileMappedFile", err)
	}
...
func (m *Mapping) Close() error {
	...
	if err := syscall.CloseHandle(m.hMappedFile); err != nil {
