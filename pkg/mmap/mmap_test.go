...
var testFilePath = filepath.Join(os.TempDir(), "./testfile.txt")
...
// TestFileOpening tests the OpenMapping function.
...
func TestFileOpening(t *testing.T) {
	...
	open := func() (*Mapping, error) {
		return OpenFileMapping(filePath, testFileMode, uintptr(testDataLength), 0, func(m *Mapping) error {
			initCallCount++
	...
	closeTestEntity(t, mf)
	mf, err = open()
	if err != nil {
		t.Fatal(err)
	}
	defer closeTestEntity(t, mf)
	if initCallCount > 1 {
