...
func TestSegmentedFile(t *testing.T) {
	...
	for i := int64(1); i < 500; i++ {
		...
		fmt.Printf("read: index=%d, key=%q, value=%q\n", i, k, v)
	}
...
func TestSegmentedFile2(t *testing.T) {
	//
	// set max file size
	maxFileSize = 2 << 10
	//
	...
	//
	// do some writing
	fmt.Printf("[--- DOING SOME WRITING ---]\n")
	for i := 0; i < 500; i++ {
		key := fmt.Sprintf("key-%04d", i+1)
		val := []byte(fmt.Sprintf("my-value-%06d", i+1))
		_, err := wal.WriteDataEntryUsingSegment(key, val)
		if err != nil {
			t.Fatalf("error writing: %v\n", err)
		}
	}
	//
	// do some reading
	fmt.Printf("[--- DOING SOME READING ---]\n")
	for i := int64(1); i < 500; i++ {
		k, v, err := wal.ReadDataEntryUsingSegment(i)
		if err != nil {
			t.Fatalf("error reading: %v\n", err)
		}
		fmt.Printf("read: index=%d, key=%q, value=%q\n", i, k, v)
	...
	//
	// do some scanning
	fmt.Printf("[--- DOING SOME SCANNING ---]\n")
	wal.Scan(func(i int64, k string, v []byte) bool {
		fmt.Printf("index=%d, key=%q, value=%q\n", i, k, v)
		return true
	})
